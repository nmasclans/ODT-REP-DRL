...
if [ "$#" -ne 11 ]; then
    echo "Usage: $0 <1_Re_tau> <2_case_name_RL> <3_realization_number_min_RL> <4_realization_number_max_RL> <5_realization_number_step_RL> <6_time_begin_averaging_non_converged> <7_time_end_averaging_non_converged> <8_time_end_averaging_converged> <9_dt_statistics> <10_nohup_filename> <11_actions_avg_freq>"
    exit 1
else
    echo -e "\n\n\n****************************************************************"
    echo -e "************************** mainRL.sh ***************************"
    ...
    echo "- Nohup filename for RL training run: ${10}"
    echo "- Actions averaging frequency, number of simulation steps for averaged actions kde: ${11}"
fi
...
echo -e "----------------------------------------------------------------\n\n\n"
python3 RL_run_actions_rewards_from_nohup.py "$2" "${10}" "${11}"
