...
for i in range( 0, len( bar_xcenters ) ):
    for j in range( 0, len( bar_ycenters ) ):
        if ( ( bar_ycenters[j] - bar_xcenters[i]*math.sqrt(3.0) ) < 0.0 ) and ( bar_ycenters[j] > 0.0 ) and ( ( bar_ycenters[j] + math.sqrt(3.0)*(bar_xcenters[i]-1.0) ) < 0.0 ): 
            if bar_h[i,j] > max_z:
                max_z = bar_h[i,j]
                max_x = bar_xcenters[i]
                max_y = bar_ycenters[j] 	
bar_x = max_x
bar_y = max_y
...
# Configure plot
#plt.xlim( 0.0, 1.0 )
#plt.xticks( np.arange( -1.0, 2.0, 10.0 ) )
#plt.tick_params( axis = 'x', bottom = True, top = True, labelbottom = 'on', labeltop = 'off', direction = 'in' )
#plt.ylim( 0.0, 1.0 )
#plt.yticks( np.arange( 0.0, 1.2, 0.2 ) )
#plt.tick_params( axis = 'y', left = True, right = True, labelleft = 'on', labelright = 'off', direction = 'in' )
plt.axis( 'off' )
ax = plt.gca()
ax.set_aspect('equal', adjustable='box')
#plt.xlabel( r'$\bar{\Delta} / \Delta$' )
#plt.ylabel( r'$\tau_{kk} / \overline{ u_{k} u_{k} }$' )
